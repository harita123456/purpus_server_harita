...
const getAllPosts = async (req, res) => {
  try {
    ...
    console.log('userWithPrivateAccount', userWithPrivateAccount.length)
    const userBlockedByOthers = await block_user.find({
      user_id: user_id,
      is_deleted: false,
    });
    ...
    console.log("usersOwnPostsNotInView", usersOwnPostsNotInView.length)
    ...
    console.log("usersOwnPostsNotInView", postWithPrivateAccountFalse.length)
    ...
    console.log("-+-+-+-+-+-+-+-+-+-+-+--+-+-+-+-+-", postWith200To300T.length)
...
const getAllPosts = async (req, res) => {
  try {
    ...
    console.log("subInterestIdss", subInterestIds)
    const postWithPrivateAccountFalse = await post
      .find({
        $and: [
          {
    ...
    /*
        const postWithPrivateAccountFalse = await post.aggregate([
          {
            $match: {
              $and: [
                { _id: { $nin: await view_post.distinct('post_id', { user_id: user_id }) } },
                {
                  $or: [
                    { sub_interest_id: { $in: subInterestIds } },
                    { sub_interest_id: null }
          ...
          },
          {
            $project: {
              ...
              "user_id._id": 1,
              "user_id.full_name": 1,
              "user_id.profile_picture": 1,
              "user_id.unique_name": 1,
              "user_id.profile_url": 1,
              "user_id.is_verified": 1,
              "user_id.is_private_account": 1,
              "interest_id": 1,
              "sub_interest_id": 1,
              "title": 1,
              "description": 1,
              ...
              "repost_user": {
                _id: "$repost_user._id",
                unique_name: "$repost_user.unique_name",
                full_name: "$repost_user.full_name",
                post_type: "$repost_user.post_type",
                profile_url: "$repost_user.profile_url",
                profile_picture: "$repost_user.profile_picture",
                is_private_account: "$repost_user.is_private_account",
                is_verified: "$repost_user.is_verified"
              }
            }
          }
        ])  */
    ...
    // const postWith200To300T = await post.aggregate([
    //   {
    //     $match: {
    //       $and: [
    //         { _id: { $nin: await view_post.distinct('post_id', { user_id: user_id }) } },
    ...
    //       "interest_id.color_code": 1,
    //       "interest_id.is_deleted": 1,
    //       "interest_id.is_block": 1,
    //       "interest_id.updatedAt": 1,
    //       "interest_id.hindi": 1,
    //       "interest_id.telugu": 1,
    //       "interest_id.kannada": 1,
    //       "interest_id.tamil": 1,
    ...
    /*   const postWithPrivateAccountTrue = await post.aggregate([
         {
           $match: {
             $and: [
               { _id: { $nin: await view_post.distinct('post_id', { user_id: user_id }) } },
         ...
         },
         {
           $project: {
             ...
             "sub_interest_id": {
               $ifNull: [
                 {
                   _id: "$sub_interest_id._id",
                   interest: "$sub_interest_id.interest",
                   color_code: "$sub_interest_id.color_code",
                   is_deleted: "$sub_interest_id.is_deleted",
                   is_block: "$sub_interest_id.is_block",
                   createdAt: "$sub_interest_id.createdAt",
                   updatedAt: "$sub_interest_id.updatedAt"
                 },
                 null
               ]
             },
   ...
   */
    ...
    if (page == 1) {
      ...
      console.log("newestPostAlgorithm_300 calling", newestPostAlgorithm_300.length)
    } else {
      console.log("-----+++++++++++++-------")
      var withoutPaginatedPosts1 = userPosts.sort((a, b) => (a.createdAt > b.createdAt ? -1 : 1));
      // If original_page1 is not 1, don't shuffle, just slice the array directly
    ...
    if (usersOwnPostsNotInView.length === 0) {
      for (let i = 0; i < 20 && i < newestPostAlgorithm_300.length; i++) {
        resultPosts.push(newestPostAlgorithm_300[i]);
      }
    } else {
      for (let i = 0; i < 10 && i < usersOwnPostsNotInView.length; i++) {
        resultPosts.push(usersOwnPostsNotInView[i]);
      }

      for (let i = 0; i < 10 && resultPosts.length < 20 && i < newestPostAlgorithm_300.length; i++) {
        resultPosts.push(newestPostAlgorithm_300[i]);
      }
    }
    ...
    if (resultPosts.length == 0) {
      ...
      if (original_page1 == 1) {
        ...
        console.log("shuffledPostsData", shuffledPostsData)
        const startIndexData = (original_page1 - 1) * parseInt(limit);
        const endIndexData = parseInt(startIndexData) + parseInt(limit);
        var userPostTrending_300_to_1000 = shuffledPostsData.slice(startIndexData, endIndexData);
      } else {
    ...
    if (resultPosts.length == 0 && userPostTrending_300_to_1000.length == 0) {
      ...
      const pageNumber = original_page2; // Assume page number from input
      const pageSize = parseInt(limit); // Assume page size from input

      // Condition to apply random sampling only on the first page
      const shouldRandomize = pageNumber == 1;
      var userPostTrending_more_than_1000 = await post.aggregate([
        {
          $match: {
            // sub_interest_id: { $in: subInterestIds },
        ...
        ...(shouldRandomize
          ? [{ $sample: { size: 10 } }] // Apply sampling only on the first page
          : [
            { $sort: { impression_count: -1, createdAt: -1 } }, // Sort in order
    ...
    if (resultPosts.length == 0 && userPostTrending_300_to_1000.length == 0 && userPostTrending_more_than_1000.length == 0) {
      ...
      const pageNumber = original_page3; // Assume page number from input
      const pageSize = parseInt(limit); // Assume page size from input
      ...
      var already_seen_posts_by_users = await post.aggregate([
        ...
        ...(shouldRandomize
          ? [{ $sample: { size: 10 } }] // Apply sampling only on the first page
          : [
            { $sort: { view_count: -1, createdAt: -1 } }, // Sort in order
            { $skip: (pageNumber - 1) * pageSize }, // Skip for pagination
            { $limit: pageSize } // Limit the results to the page size
          ]
        ),
    ...
    if (!selected_id) {
      if (resultPosts.length > 0) {
        const post_for_display = await processUserPosts(resultPosts, user_id);
        console.log("resultPosts", post_for_display.length)
        return multiSuccessRes(
          res,
          "Posts retrieved successfully",
          post_for_display,
    ...
    if (selected_id) {
      ...
      console.log("subinterestCountResult", subinterestCountResult)
      if (subinterestCountResult.length > 0) {
        /* const postWithPrivateAccountFalse = await post
           .find({
             $and: [
        ...
        const postWithPrivateAccountFalse = await post.aggregate([
          ...
          },
          {
            $project: {
              ...
              "user_id.full_name": "$user.full_name",
              "user_id.profile_picture": "$user.profile_picture",
              "user_id.unique_name": "$user.unique_name",
              "user_id.profile_url": "$user.profile_url",
              "user_id.is_verified": "$user.is_verified",
              "user_id.is_private_account": "$user.is_private_account",
              "interest_id._id": "$interest._id",
              "interest_id.interest": "$interest.interest",
              "interest_id.color_code": "$interest.color_code",
              "interest_id.is_deleted": "$interest.is_deleted",
              "interest_id.is_block": "$interest.is_block",
              "interest_id.createdAt": "$interest.createdAt",
              "interest_id.updatedAt": "$interest.updatedAt",
              "sub_interest_id._id": "$sub_interest._id",
              "sub_interest_id.sub_interest": "$sub_interest.sub_interest",
              "sub_interest_id.color_code": "$sub_interest.color_code",
              "sub_interest_id.is_deleted": "$sub_interest.is_deleted",
              "sub_interest_id.is_block": "$sub_interest.is_block",
              "sub_interest_id.createdAt": "$sub_interest.createdAt",
              "sub_interest_id.updatedAt": "$sub_interest.updatedAt",
              "title": 1,
              "description": 1,
              "post_type": 1,
              "link_url": 1,
          ...
          },
          {
            $project: {
              ...
              "interest_id": {
                ...
                "color_code": "$interest_id.color_code",
                "is_deleted": "$interest_id.is_deleted",
                "is_block": "$interest_id.is_block",
                "createdAt": "$interest_id.createdAt",
                "updatedAt": "$interest_id.updatedAt"
              },
              "sub_interest_id": {
                "_id": "$sub_interest_id._id",
                "interest_id": "$sub_interest_id.interest_id",
                "sub_interest": "$sub_interest_id.sub_interest",
                "color_code": "$sub_interest_id.color_code",
                "is_deleted": "$sub_interest_id.is_deleted",
                "is_block": "$sub_interest_id.is_block",
                "createdAt": "$sub_interest_id.createdAt",
        ...
        // return successRes(res, `Interests get successfully`, postWithPrivateAccountTrue);
        const filteredPosts = postWithPrivateAccountTrue.filter(
          (post) =>
            post.user_id.is_private_account === false &&
            (!post.repost_id ||
        ...
        if (resultPosts.length == 0 && userPostTrending_300_to_1000.length == 0 && userPostTrending_more_than_1000.length == 0) {
          ...
          console.log("userPostTrending_others_data", userPostTrending_others_data.length)
        }
        if (resultPosts.length > 0) {
          const post_for_display = await processUserPosts(resultPosts, user_id);
        ...
        if (userPostTrending_300_to_1000.length > 0) {
          const post_for_display = await processUserPostsAggree(userPostTrending_300_to_1000, user_id);
          console.log("userPostTrending_300_to_1000", userPostTrending_300_to_1000.length)
          return multiSuccessRes(
            res,
            "Posts retrieved successfully",
            post_for_display,
        ...
        if (userPostTrending_more_than_1000.length > 0) {
          const post_for_display = await processUserPostsAggree(userPostTrending_more_than_1000, user_id);
          console.log("userPostTrending_more_than_1000", userPostTrending_more_than_1000.length)
          return multiSuccessRes(
            res,
            "Posts retrieved successfully",
            post_for_display,
        ...
        const pageNumber = page; // Assume page number from input
        const pageSize = parseInt(limit);
        const shouldRandomize = pageNumber === 1;

        console.log("calling last else", pageNumber, pageSize)
        var userPosts = await post.aggregate([
          {
            $match: {
              user_id: { $nin: blockedUserIds },
...
const getAllPostsaggregation = async (req, res) => {
  try {
    if (!req.body.user_id) {
      var user_id = req.user._id;
    } else {
...
const getAllPostsBySubInterest = async (req, res) => {
  try {
    ...
    if (selected_id) {
      ...
      console.log("192.168.29.234", queryObject.sub_interest_id)
      /*
           const userPostsPrivateFalse = await post.aggregate([
             {
               $match: {
                 $and: [
                   // {
     ...
     */
      ...
      console.log("192.168.29.234ssss", queryObject.sub_interest_id)
      ...
      /*
          const userPostsPrivateTrue = await post.aggregate([
            {
              $match: {
                $and: [
                  {
      ...
      if (userPostsCount === 0) {
        ...
        userPosts.forEach(async (post) => {
          ...
          if (post?.repost_id?.post_media.length > 0) {
            post?.repost_id?.post_media.forEach((media) => {
              if (media.file_type === "image" || media.file_type === "video") {
                media.file_name = process.env.BASE_URL + media.file_name;
                if (media.thumb_name) {
...
const getAllPostsBySubInterestAggregtion = async (req, res) => {
  try {
    // if (!req.body.user_id) {
    //   var user_id = req.user._id;
    // } else {
...
module.exports = {
  ...
  getAllPostsBySubInterestAggregtion
};
